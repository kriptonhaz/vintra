...
import {
  ...
  toggleMyCode,
  getMyCodeBudget,
} from '@/server/functions/marketing-codes'
...
export const Route = createFileRoute('/_authed/marketing/')({
  loader: async () => ({
    codes: await listMyCodes(),
    budget: await getMyCodeBudget(),
  }),
  component: MarketingCodesPage,
})

type MarketingCode = Awaited<ReturnType<typeof listMyCodes>>[number]

function MarketingCodesPage() {
  ...
  const { data: budgetData = initial.budget } = useQuery({
    queryKey: ['marketing', 'code-budget'],
    queryFn: () => getMyCodeBudget(),
    initialData: initial.budget,
    staleTime: 5 * 60_000,
  })
  const budget = budgetData.budgetPct

  const [showCreateSheet, setShowCreateSheet] = useState(false)
  const [editingCode, setEditingCode] = useState<MarketingCode | null>(null)
  const [sharingCode, setSharingCode] = useState<MarketingCode | null>(null)
  const [mutationLoading, setMutationLoading] = useState(false)
  const [copiedId, setCopiedId] = useState<string | null>(null)

  async function refresh() {
    queryClient.invalidateQueries({ queryKey: ['marketing', 'codes'] })
  ...
  async function handleToggle(code: MarketingCode) {
    try {
      await toggleMyCode({ data: { id: code.id, isActive: !code.isActive } })
      await refresh()
    } catch (err) {
  ...
  async function handleCopy(code: MarketingCode) {
    try {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <CardTitle>Kode Referral Saya</CardTitle>
              <CardDescription className="mt-1">
                Buat dan kelola kode yang akan Anda bagikan ke calon pelanggan.
              </CardDescription>
            </div>
            <Button
              variant="brand"
          ...
          {codes.length === 0 ? (
            <div className="flex flex-col items-center py-12">
              ...
              <p className="mt-1 max-w-md text-center text-sm text-gray-500 dark:text-gray-400">
                Buat kode pertama Anda. Bagikan ke calon pelanggan lewat WhatsApp atau
                Instagram, dan dapatkan komisi setiap kali mereka berlangganan.
              </p>
            </div>
          ) : (
            <Table>
                      ...
                      <div className="inline-flex items-center gap-1.5">
                        {code.attributionCount > 0 ? (
                          <Link
                            ...
                            aria-label={`Lihat pendaftar kode ${code.code}`}
                          >
      ...
      <Sheet open={showCreateSheet} onClose={() => setShowCreateSheet(false)}>
        <SheetHeader onClose={() => setShowCreateSheet(false)}>
          ...
          <SheetDescription>
            Atur kode unik dan bagaimana budget {budget.toFixed(0)}% dibagi antara
            diskon untuk pendaftar dan komisi untuk Anda.
          </SheetDescription>
        </SheetHeader>
        <CodeForm
          mode="create"
...
// Default split: 75% of the budget to discount, 25% to commission.
...
function CodeForm({ mode, cap, defaultValues, onSubmit, onCancel, loading }: CodeFormProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (overCap) next.commissionPct = `Total melebihi budget ${cap}%`
...
function ShareDialog({ code, onClose }: ShareDialogProps) {
  ...
  const message =
    `Halo! Kenalin Vintra — aplikasi buat kelola usaha: HPP, POS, inventaris, sampai absensi karyawan, semua dalam satu app.\n\n` +
    `Pakai kode *${code.code}* saat daftar, dapat diskon ${discountPct}% buat berlangganan modul berbayar.\n\n` +
  ...
  async function handleNativeShare() {
    try {
      await navigator.share({ title: 'Vintra', text: message, url: shareUrl })
    } catch (err) {
      ...
      <DialogHeader>
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1">
            ...
            <DialogDescription>
              Bagikan tautan ini ke calon pelanggan — kode akan otomatis terisi saat mereka daftar.
            </DialogDescription>
          </div>
          <button
            type="button"
