...
export const Route = createFileRoute('/_authed/referrals/commission')({
  loader: async () => ({
...
function CommissionDashboard() {
  ...
  async function refresh() {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: ['tenant', 'referral-commission-summary'] }),
      queryClient.invalidateQueries({ queryKey: ['tenant', 'referral-commissions'] }),
      queryClient.invalidateQueries({ queryKey: ['tenant', 'referral-claim-requests'] }),
      queryClient.invalidateQueries({ queryKey: ['tenant', 'referral-payout-method'] }),
    ])
  ...
  async function handleSubmitClaim() {
    ...
    try {
      ...
      // Auto-close the confirmation dialog on error so the user
      // doesn't have to manually dismiss it before retrying — the
      // toast carries the failure reason.
      setClaimDialogOpen(false)
      toast({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <StatTile label="Dalam Masa Klaim" value={summary.pending} icon={<Clock className="h-5 w-5" />} tooltip="Komisi akan tersedia diklaim setelah masa klaim selesai" />
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center text-sm text-gray-500 dark:text-gray-400">
          Belum ada komisi. Komisi akan muncul di sini saat pendaftar dari kode Anda mulai berlangganan.
        </CardContent>
      </Card>
    )
  }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      await upsertPayoutMethod({
        data: { bankName, accountNumber, accountHolderName },
      })
