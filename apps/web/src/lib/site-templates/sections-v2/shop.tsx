...
/**
 * Renders a single track-order lookup result. Used by the `/track` page
 * (the situs "Lacak pesanan" link now routes there instead of opening a
 * modal). Pass the resolved `trackOrder` result; renders a not-found
 * message when `result.found` is false.
 */
export function TrackResultView({
  result,
  brandColor,
}: {
  result: NonNullable<Awaited<ReturnType<typeof trackOrder>>>
  brandColor: string
}) {
  if (!result.found) {
    return (
    ...
    <div className="space-y-3">
      ...
      {result.trackingNumber && (
        <p className="rounded-lg bg-gray-50 p-2 text-sm">
          ...
          <span className="font-medium tabular-nums">{result.trackingNumber}</span>
        </p>
      )}
