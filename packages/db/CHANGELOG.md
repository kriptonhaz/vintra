...
- a2f8a1f: Phase 3 — WhatsApp per-branch foundation. Add a nullable `branch_id` to `wa_instances` (migration `0090`) so a franchise can run a separate WhatsApp number per outlet, sold as a per-branch add-on. Existing instances stay tenant-level (`branch_id` null).
...
- a2f8a1f: Add a composite index on `attendance_records (tenant_id, date)`. The records list and count queries filter by tenant + date range and order by date; the index serves both without a sequential scan as daily clock-in rows accumulate.
