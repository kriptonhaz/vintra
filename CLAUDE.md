...
12. **DO NOT use `drizzle-kit push`** — Crashes on Supabase (CHECK constraint bug). **`db:generate` is also broken** (corrupt meta snapshots) — hand-write migrations + `bun run db:migrate`, then `bun run check:drift`. See "Database Migration Workflow"
...
drizzle-kit bug parsing CHECK constraints from Supabase system schemas.

**`bun run db:generate` DOES NOT WORK EITHER (since June 2026).** It errors with
a snapshot collision: `meta/0004_snapshot.json` and `meta/0005_snapshot.json` are
byte-identical, sharing an `id` and a `prevId`. Only 6 snapshots exist for 144
migrations, so drizzle's model of the schema is frozen at 19 tables while the real
schema has 127.

Do **not** "fix" this by deleting the duplicate snapshot. That would make
`db:generate` run again — and immediately emit a migration creating the ~108
tables it thinks are missing, all of which already exist. The broken tool is
currently the only thing preventing that.

**The actual workflow — every migration since 0006 has used it:**

1. Edit schema files in `packages/db/src/schema/`
2. **Hand-write** the SQL migration in `packages/db/src/migrations/`, using the
   next number, `--> statement-breakpoint` between statements, and idempotent
   DDL (`IF NOT EXISTS`) so it is safe to re-apply
3. Append a matching entry to `packages/db/src/migrations/meta/_journal.json`
4. Run `bun run db:migrate` to apply pending migrations
5. Run `bun run check:drift` to confirm the schema files and the database still
   agree — this is the only check that the hand-written SQL matched the schema
...
**Re-baselining** (squashing the 144 migrations into one generated baseline so
`db:generate` works again) is possible but must be gated on `bun run check:drift`
reporting zero drift first. As of 2026-08-17 it does — 127 tables and 1377 columns
match exactly. On JuraganQu they did not, and a legacy `materials.supplier` column
that no migration created broke a dump restore.
